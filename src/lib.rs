...
impl Distribution<u16> for Standard {
        ...
        R: Rng + ?Sized,
    {
        match bits {
            ...
            u16::BITS => u16::MAX,
            _ => {
                assert!((1..u16::BITS).contains(&bits), "bits count out of range");

                let min_low_bits = bits.checked_sub(u8::BITS).unwrap_or_default();
                let max_low_bits = min(bits, u8::BITS);
...
impl Distribution<u32> for Standard {
        ...
        R: Rng + ?Sized,
    {
        match bits {
            ...
            u32::BITS => u32::MAX,
            _ => {
                assert!((1..u32::BITS).contains(&bits), "bits count out of range");

                let min_low_bits = bits.checked_sub(u16::BITS).unwrap_or_default();
                let max_low_bits = min(bits, u16::BITS);
                let low_bits = rng.gen_range(min_low_bits..=max_low_bits);
                let high_bits = bits - low_bits;
...
impl Distribution<u64> for Standard {
        ...
        R: Rng + ?Sized,
    {
        match bits {
            ...
            u64::BITS => u64::MAX,
            _ => {
                assert!((1..u64::BITS).contains(&bits), "bits count out of range");

                let min_low_bits = bits.checked_sub(u32::BITS).unwrap_or_default();
                let max_low_bits = min(bits, u32::BITS);
                let low_bits = rng.gen_range(min_low_bits..=max_low_bits);
                let high_bits = bits - low_bits;
